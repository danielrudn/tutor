...
class PluginsTests(PluginsTestCase):
    ...
    def test_plugin_without_patches(self) -> None:
        plugins_v0.DictPlugin({"name": "plugin1"}).enable()
        patches = list(plugins.iter_patches("patch1"))
    ...
    def test_dict_plugin(self) -> None:
        ...
        plugin.enable()
        config: Config = filters.apply("config:overrides", {})
        self.assertEqual("myplugin", plugin.name)
        self.assertEqual("0.1", plugin.version)
        self.assertEqual({"KEY": "value"}, config)

    def test_config_disable_plugin(self) -> None:
        plugin1 = plugins_v0.DictPlugin(
            {"name": "myplugin", "config": {"set": {"KEY1": "value1"}}}
        )
        plugin2 = plugins_v0.DictPlugin(
            {"name": "myplugin", "config": {"set": {"KEY2": "value2"}}}
        )
        plugin1.enable()
        plugin2.enable()
